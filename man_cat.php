...
<div class="row">
 ...
 $msg="";
	 ...
	 $mq=mysql_query("select max(id) from sc_category");
	 $mr=mysql_fetch_array($mq);
	 $id=$mr['max(id)']+1;
	 
	
	 $ins=mysql_query("insert into sc_category(id,category) values($id,'$category')");
		 if($ins)
		 {
		 ?>
		 <script language="javascript">
		 window.location.href="man_cat.php?act=success";
		 </script>
		 <?php
		 }
		 else
		 ...
		 ?>
		 <script language="javascript">
		 window.location.href="man_cat.php?act=wrong";
		 </script>
		 <?php
		 }
...
<!--end content area-->
  <p>&nbsp;</p>
  <?php
  $qry=mysql_query("select * from sc_category");
